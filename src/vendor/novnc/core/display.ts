...
export default class Display {
  ...
  constructor(target: HTMLCanvasElement) {
    ...
    this._targetCtx = require2dContext(this._target);

    // The hidden canvas, where we do the actual rendering
    this._backbuffer = document.createElement("canvas");
    this._drawCtx = require2dContext(this._backbuffer);
  ...
  flip(fromQueue?: boolean): void {
    if (this._renderQ.length !== 0 && !fromQueue) {
      ...
      if (w > 0 && h > 0) {
        ...
        this._targetCtx.drawImage(this._backbuffer, x, y, w, h, x, y, w, h);
      }
  ...
  copyImage(
    ...
    if (this._renderQ.length !== 0 && !fromQueue) {
      ...
      const legacyCtx = this._drawCtx as CanvasRenderingContext2D &
        Record<
  ...
  _resumeRenderQ(this: HTMLImageElement): void {
    ...
    this.removeEventListener("load", display._resumeRenderQ);
